# start the training job asynchronously so it runs in the background while you track its status
...
try:
    # TODO: Start the training job asynchronously using the fit() method
    sklearn_estimator.fit({'train': S3_TRAIN_DATA_URI}, wait=False) # wait=False makes it asynchronous
    
    # TODO: Get the training job name from the estimator
    training_job_name = sklearn_estimator.latest_training_job.name
    
    # TODO: Print the training job name
    print(f"Training job started: {training_job_name}")
    
    # TODO: Get the current job status from the training job description
    job_status = sklearn_estimator.latest_training_job.describe()['TrainingJobStatus']
    
    # TODO: Print the current job status
