# run the provided performance comparison script, which will measure and display the response times of both a serverless endpoint (california-housing-estimator) and your real-time endpoint (california-housing-realtime) using identical test data
...
try:
    ...
    print(f"Performance Difference: {abs(serverless_time - realtime_time):.4f} seconds")
    
    if realtime_time < serverless_time:
        print(f"Real-time endpoint was {serverless_time/realtime_time:.2f}x faster")
    else:
        print(f"Serverless endpoint was {realtime_time/serverless_time:.2f}x faster")
